 ...
 * @param const string & rawTx createrawtx 명령어를 통하여 만들어진
 * sign 된 16진수 Hex rawtransaction 문자열
 *
 ...
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const string & address 정보를 가져올 특정 주소 문자열
 * @param bool rescan 특정 지갑주소의 정보를 가져올 때, rescan 을
 * 수행 할 것인지 아닌지에 대한 여부 (default = true)
 *
 ...
 * @details rescan 을 통하여 특정 주소의 정보를 가져 온다.
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const vector<string> & address 정보를 가져올 특정 주소 문자열
 * @param bool rescan 특정 지갑주소의 정보를 가져올 때, rescan 을
 * 수행 할 것인지 아닌지에 대한 여부 (default = true)
 *
 * @return importaddress RPC 명령어의 결과 값, (없음)
 ...
 * @details HDAC 네트워크의 버전, 블럭 높이, 체인 이름 등의
 * 기본 정보를 가져 온다.
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 *
 ...
 * @details HDAC 네트워크의 지갑 주소 체크섬, 체인 이름, rpc port 등의
 * 파라미터 설정 정보를 가져 온다.
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 *
 * @return blockchainparams RPC 명령어의 결과 값, (지갑 주소 체크섬,
 * 체인 이름, rpc port 등의 HDAC 네트워크의 파라미터 설정 정보)
 *
 */
 ...
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const vector<string> & addresses 조회하고자 하는 특정 지갑 주소들
 *
 * @return listunspent RPC 명령어의 결과 값, (특정 지갑에 보내지 못한
 * transaction 리스트)
 *
 */
 ...
 * transaction 들의 정보를 가져 온다.
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const string & addresses 조회하고자 하는 특정 지갑 주소들
 *
 * @return listunspent RPC 명령어의 결과 값, (특정 지갑에 보내지 못한
 * transaction 리스트)
 *
 ...
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param bool unlock 잠금 할 껀지,  잠금 해제 할 껀지  여부
 * @param string txid 잠금 또는 잠금 해제 할 transaction 의 ID
 * @param int vout 잠금 또는 잠금 해제 하고자 하는 transaction 의 vout 양
 *
 ...
 * transaction 들의 정보를 가져 온다.
 * @details 아직 보내지 못한 transaction 들 중에 전송 되지 못하도록 잠겨 있는
 * transaction 들의 정버를 가져 온다.
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 *
 * @return listlockunspent RPC 명령어의 결과 값, (잠금 되어진 transaction 들의
 * ID 와 vout 값)
 *
 */
 ...
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const vector<string> & streamName 특정 stream 의 정보를 가져
 *  올 때 사용 한다.
 *
 ...
 * @details 생성 되어진 stream 들의 정보를 가져 온다.
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const string & streamName 특정 stream 의 정보를 가져 올 때 사용 한다.
 *
 * @return liststreams RPC 명령어의 결과 값, (생성되어진 stream 정보)
 *
 */
 ...
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const vector<string> & assetNames 특정 asset 들의 정보를 가져
 * 올 때 사용 한다.
 *
 ...
 * @details 정의 되어진 asset 들의 정보를 가져 온다.
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const string & name 특정 asset 들의 정보를 가져
 * 올 때 사용 한다.
 *
 * @return listassets RPC 명령어의 결과 값, (생성되어진 asset 정보)
 *
 ...
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const string & strPrivateKey sign 할려고하는 개인키 값
 * @param const string & strText sign 할려고하는 평문의 문자열
 *
 ...
 * @details signmessage 를 통하여 sign 된 메시지가 제대로 sign 되었는지
 * 검증 한다.
 * @param const RpcClient & client HDAC 네트워크 접속 정보를 제공한다.
 * @param const string & strAddress sign 할 때 사용 된 지갑 주소
 * @param const string & strSignmessage sign 되어진 문자열
 * @param const string & strText sign 한 원본 문자열
 *
 ...
 * 변환 시켜 준다.
 * @param const Object & reply RPC 명령어로 반환 된 json 형태의 결과 값
 * @param string * resultStr string 형으로 변환 된 결과 값
 *
